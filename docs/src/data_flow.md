...
`channel!` does not return the endpoints you passed in - it returns *instrumented wrappers* around them. The macro expands to a different type than the original:
...
- `hotpath::wrap::tokio::sync::oneshot::{Sender, Receiver}`
