...
/// `HOTPATH_META_PROMETHEUS_FAST_BUCKETS` (seconds, comma-separated) or
/// `DEFAULT_FAST_LADDER_NS`.
pub(crate) static FAST_LADDER_NS: LazyLock<Vec<u64>> = LazyLock::new(|| {
    ladder_from_env(
        "HOTPATH_META_PROMETHEUS_FAST_BUCKETS",
        DEFAULT_FAST_LADDER_NS,
    )
});

/// Classic ladder for SQL, HTTP client and server durations:
/// `HOTPATH_META_PROMETHEUS_SLOW_BUCKETS` (seconds, comma-separated) or
/// `DEFAULT_SLOW_LADDER_NS`.
pub(crate) static SLOW_LADDER_NS: LazyLock<Vec<u64>> = LazyLock::new(|| {
    ladder_from_env(
        "HOTPATH_META_PROMETHEUS_SLOW_BUCKETS",
        DEFAULT_SLOW_LADDER_NS,
    )
});
