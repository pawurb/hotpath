...
impl<'a> MetricsProvider<'a> for StatsData<'a> {
    ...
    fn description(&self) -> String {
        "Peak memory usage (maximum bytes allocated at any point) during each function call."
            .to_string()
    }
    ...
    fn metric_data(&self) -> HashMap<String, Vec<MetricType>> {
        ...
        let grand_total_bytes: u64 = filtered_stats
            .iter()
            ...
            .map(|(function_name, stats)| {
                ...
                for &p in &self.percentiles {
                    if stats.has_unsupported_async {
                        ...
                        metrics.push(MetricType::AllocBytes(bytes_max));
                    }
                }
