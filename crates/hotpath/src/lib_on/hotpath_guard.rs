...
/// `MAX_PERCENTILES` in `hotpath-macros`.
...
impl HotpathGuardBuilder {
    ...
    /// Values are sorted and deduplicated, and at most 10 distinct percentiles
    /// are allowed - each one adds a column to every report table.
